...
(function() {
  ...
  LArray = (function(_super) {
    ...
    LArray.mapper = function(fn, subArgs) {
      ...
      return ss.json(through(function(update) {
        var childUpdate, data, key, mapper, sbKey, _ref1,
          _this = this;
        if (Array.isArray(update)) {
          ...
          switch (data[0]) {
            case 'c':
              ...
              return db[data[1]].write(childUpdate);
            case 'd':
              mapper = (_ref1 = LBase.types[data[2]]).mapper.apply(_ref1, [fn].concat(__slice.call(subArgs)));
              mapper.on('data', function(update) {
                return _this.queue([['c', data[1], update[1]], update[2][1], update[2][2]]);
              });
              db[data[1]] = mapper;
              return this.queue([['d', data[1], data[2], LBase.types[data[2]].mapCreationArgs(fn, data[3])], update[1], update[2]]);
            default:
              for (sbKey in data) {
                key = data[sbKey];
                if (key != null) {
  ...
  LValue = (function(_super) {
    ...
    LValue.mapper = function(fn) {
      return ss.json(through(function(update) {
        var newUpdate;
        return this.queue(Array.isArray(update) ? (newUpdate = [fn(update[0]), update[1], update[2]], newUpdate.custom = update.custom, newUpdate) : update);
      }));
    };
